...
- [x] **모바일 룸 레이아웃 1차 개선** — 상태별 `NEXT MOVE` 안내, 모바일 하단 액션바, 채팅 점프, YouTube "탭해서 재생" 힌트 추가. `typecheck` + `build` 통과.
- → [ ] **모바일 룸 실기기 검증** — 본인 폰으로 룸 페이지 핵심 행동(DJ 참여 / 곡 추가 / 채팅 / 반응) 도달 가능성 확인. 깨지는 곳 스크린샷.
